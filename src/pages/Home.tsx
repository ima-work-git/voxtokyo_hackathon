...
export default function Home() {
  ...
  const [chatMode, setChatMode] = useState<'dispatch' | 'chat'>('dispatch')
...
spoken_text は「結論（119/110）→短い理由→不足質問（ある場合は最大2つ）」を1つにまとめた短文にしてください。`,
  ...
  useEffect(() => {
    ...
    if (voiceAutoSend) {
      if (chatMode === 'dispatch') void onEmergencyTriage(t)
      else void onSendChat(t)
      return
    }

    setChatInput((p) => (p ? `${p} ${t}` : t))
  }, [chatMode, dictation, voiceAutoSend])
        ...
        <section className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex items-center gap-1 rounded-xl border border-zinc-800 bg-zinc-950 p-1">
              <button
                type="button"
                onClick={() => setChatMode('dispatch')}
                className={
                  chatMode === 'dispatch'
                    ? 'rounded-lg bg-zinc-100 px-2 py-1 text-xs font-semibold text-zinc-950'
                    : 'rounded-lg px-2 py-1 text-xs font-semibold text-zinc-200 hover:bg-zinc-900'
                }
              >
                指令員
              </button>
          ...
          <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center">
            <div className="flex flex-1 items-center gap-2 rounded-xl border border-zinc-800 bg-zinc-950 px-3 py-2">
              <input
                ...
                placeholder={chatMode === 'dispatch' ? '通報内容を入力…（例: 血が止まらない / 火事です / ストーカーがいます）' : 'Type your message…'}
                className="w-full bg-transparent text-sm text-zinc-100 outline-none placeholder:text-zinc-600"
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !e.shiftKey) {
                    e.preventDefault()
                    if (chatMode === 'dispatch') onEmergencyTriage()
                    else onSendChat()
                  }
                }}
              />
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {chatBusy ? '送信中…' : chatMode === 'dispatch' ? '指令員へ送信' : '送信'}
              </button>
